    ...
    def _check_lineweights(self, dxf_data: Dict[str, Any]):
        ...
        for entity_type in ['LINE', 'CIRCLE', 'ARC', 'POLYLINE']:
            for entity in dxf_data['entities'].get(entity_type, []):
                ...
                if not is_standard:
                    # 构建详细的实体信息
                    recommended_weight = min(standard_weights, key=lambda x: abs(x-lineweight_mm))
                    entity_details = {
                        "entity_type": entity_type,
                        "current_lineweight": lineweight_mm,
                        "standard_lineweights": standard_weights,
                        "recommended_lineweight": recommended_weight,
                        "tolerance": tolerance
                    }
                    
                    # 构建更详细的描述
                    detail_info = f"当前线宽: {lineweight_mm:.2f}mm, 推荐: {recommended_weight:.2f}mm"
                    ...
                    self.violations.append(Violation(
                        ...
                        layer=entity['layer'],
                        entity_details=entity_details,
                        suggestion=f"使用标准线宽: {recommended_weight}mm"
                    ))
